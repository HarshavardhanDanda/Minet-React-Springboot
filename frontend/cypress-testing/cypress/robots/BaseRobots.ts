...
export abstract class BaseEyes {
  ...
  seesTextWithTextLabel(text: string) {
...
export class BaseHands {
  ...
  dropDown(dom: string, select: string) {
    cy.get(dom).select(select, { force: true });
    return this;
  }
...
export class BaseDependencies {
  ...
  homeUrl() {
    cy.visit(`https://bc88-fe.fe-assignment.tk`);
    return this;
  }
  ...
  login() {
    ...
    cy.get("[placeholder='Enter Password'").type("Minet1@password.com");
  ...
  falseLogIn() {
    cy.fixture("user_data.json").then((data) => {
      ...
      cy.get('input[type="password"]').type(data.false_password);
      cy.contains("Sign In").click();
    });
  }
  falseSignIn() {
    cy.fixture("user_data.json").then((data) => {
      cy.get('[placeholder="Eg: John Doe"]').type(data.full_name);
      cy.get('[placeholder="you@company.com"]').type(data.false_email);
      cy.get('input[type="password"]').type(data.false_password);
